...
def dateselector():
    """ This Definition lets the user select the dates of which to look at. Note that the dates mus be entered in the
    YYYY-MM-DD Format"""
...
def save_imported_date(stationname, datalist):
    """This Takes the Data and Save's it to a CSV file"""
    ...
    try:
        ...
        if savefile == 'y' or savefile == 'Y' or savefile == 'Yes' or savefile == 'yes':  # If yes save the file
            ...
            for i in datalist:
                ...
                packstring += i[1].strftime('%m/%d/%Y') + '\n'  # Dates (which have been convered to a string
...
def runprogram():
    ...
    print("Let's Check Out: ", station_name_call)
    ...
    save_imported_date(station_name_call, snowdata[0])
