...
if [float(i) for i in pd.__version__.split(".")] < [1, 0, 0]:
    DATETIME_TYPES = (_datetime, np.datetime64, pd.datetime, pd.DatetimeIndex)
else:
    DATETIME_TYPES = (_datetime, np.datetime64)
...
STRING_TYPES = tuple([str] + [str, np.str, np.unicode, chr])
...
def sanitize_field_name(s, length=None, sub_value=None):
    ...
    if isinstance(length, int) and length > 0 and len(s) > length:
        s = s[:length]
    return s
