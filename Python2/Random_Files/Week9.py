...
You are to write a function called recAdd that takes
in a list of numbers and returns their sum.
...
def recAdd(list):
    ...
    if len(list) <= 1:
        ...
        sum += list[0] + recAdd(list[1:])
        return sum
# print(recAdd([3, 2, 1, 6, 3, 8]))
'''2. palindrome

You are to write a function called isPalindrome 
that takes a string as a parameter.  
It should return True is the string is a 
palindrome and False if it isn't. 
For this problem you must write the 
function recursively.

You should check your function for "racecar", 
"noon", and "abca".  For example:
...
def isPalindrome(str):
    ...
    '''
    #look at string forwards,
    #look at string backwards
    #should be the same
    if len(str) <= 1:
        return True
    elif str[0] != str[-1]:
...
def isPalindromeIter(str):
    ...
    for i in range(len(str)):
        if str[0+i] != str[-1-i]:
            return False
    return True
# print(isPalindromeIter('racecar'))

'''3. fileCount

For this problem, you will write a recursive 
function called fileCount.  fileCount takes as a 
parameter a string path to a directory on your filesystem.  
You may assume this directory path is correct.  For example:
#Absolute path
fileCount("/Users/dcrutchfield/Desktop/DU/Teaching/COMP3005")
or
# Relative path
fileCount("../../assignments/assignment8")
Your function should count the number of files 
in that directory and return that number  
You can obtain all the files/directories in a directory 
with the command:
os.listdir(pathToDir)
where pathToDir is a string path to a directory.  
You can simply iterate though them to look at each one. 
 In order to use any of the os commands, you will need to:
import os
You can then use os.path.isfile(path) or 
os.path.isdir(path) to determine if a given string path 
is a file or a directory - they both return True/False.

This should be a recursive function since we not 
only want a count of the files in the given directory, 
but also in all the sub-directories, and their 
sub-directories, etc.

Hint: I would suggest getting the function to work without the recursive decent.  That is, just have it return how many files it finds in the given directory and not look at the sub-directories.  Once you have that working, then you can add the recursion.

Hint: all the os.* functions described above need a full path to the file/directory.  Make sure you construct a full path (relative or absolute) to the file or directory in question.
'''
import os
def fileCountIter(filePath):
    ...
    for file in files:
        if os.path.isfile(file):
            ...
            #keep looping until no sub folders
    return fileCount
def fileCount(filePath):
    count = 0
    if os.path.isfile(filePath):
            return 1
    #otherwise we are a directory
    else:
        files = os.listdir(filePath)
        for file in files:
            count += fileCount(filePath+'\\'+file)
        return count
fileLoc = r'C:\Class9'
# print(fileCount(fileLoc))
...
def fileList(filePath):
    ...
    #otherwise we are a directory
        ...
        files = os.listdir(filePath)
        for file in files:
            list += fileList(filePath+'\\'+file)
        return list
fileLoc = 'C:\Class9'
# print(fileList(fileLoc))

'''4. boggle

For this problem, you will be creating code to 
help with playing the game boggle.  
The basic idea is given a set of letters,
 say {'a', 't', 'c'}, find all the words 
 that results from any rearrangement of these letters. 
  For this problem, we will assume duplicate letters 
  cannot exist in the original given list.  
  Thus, a we will use a set to hold our letters.  
  So for the example above:

starting = {'a', 't', 'c'}

You may just hardcode a starting set of 
letters for this problem.

The next thing you need to do is to create a 
function called perm that takes in the set of letters 
and returns every possible string.  For example:
...
def perm(letters):
    ...
    if len(letters) <=1: #base case
        ...
        for i in range(len(letters)):
            for p in perm(letters[:i]+letters[i+1:]):
                perms.append(letters[i] + p)
                '''first letter c
                    perms = [at, ta]
                    ->[cat, cta]
                    first letter t
